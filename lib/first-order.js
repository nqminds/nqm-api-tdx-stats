module.exports = (function() {
  ...
  const getBasic = function(types, datasetId, match, fields, index, limit, timeout) {
    ...
    _.forEach(types, (type) => {
      // Extract the projectcion fields from the type and fields parameters
    ...
    const group = helper.getGroup(types, fields);
  ...
  const getMin = function(datasetId, match, fields, timeout) {
    const params = {
      type: [{"$min": constants.fieldStrKey}],
      match: match,
      fields: fields,
      timeout: timeout,
    };
  ...
  const getMax = function(datasetId, match, fields, timeout) {
    const params = {
      type: [{"$max": constants.fieldStrKey}],
      match: match,
      fields: fields,
      timeout: timeout,
    };
  ...
  const getSum = function(datasetId, match, fields, timeout) {
    const params = {
      type: [{"$sum": constants.fieldStrKey}],
      match: match,
      fields: fields,
      timeout: timeout,
    };
  ...
  const getAvg = function(datasetId, match, fields, timeout) {
    const params = {
      type: [{"$avg": constants.fieldStrKey}],
      match: match,
      fields: fields,
      timeout: timeout,
    };
  ...
  const getStdSample = function(datasetId, match, fields, timeout) {
    const params = {
      type: [{"$stdDevSamp": constants.fieldStrKey}],
      match: match,
      fields: fields,
      timeout: timeout,
    };
  ...
  const getStdPopulation = function(datasetId, match, fields, timeout) {
    const params = {
      type: [{"$stdDevPop": constants.fieldStrKey}],
      match: match,
      fields: fields,
      timeout: timeout,
    };
