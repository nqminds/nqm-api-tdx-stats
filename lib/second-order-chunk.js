module.exports = (function() {
  ...
  const iterateHistogramChunks = function(datasetId, params, cf, matched) {
    ...
    return getChunkIterator({datasetId: datasetId, params: params}, getBasicBins, matched)
            .then((iterator) => {
  ...
  const getHistogramChunk = function(datasetId, params, cf) {
    ...
    // Assign a zero value as the count for Promise and set matched to false
    let retPromise = Promise.resolve({count: 0, matched: false});
    ...
    return retPromise.then((result) => {
      if (!result.matched || (result.matched && result.count)) {
        return iterateHistogramChunks(datasetId, params, cf, result);
      } else if (result.matched && !result.count){
        const retObj = {
          count: 0,
          bins: [],
          binIndex: params.binIndex,
