module.exports = (function() {
    ...
    if (_.isEmpty(tdxObject))
      ...
      if (type.length) {
        _.forEach(fields, (val) => {
          retObj[val] = new Array(type.length);
        });
      }
    }
  ...
  // Creates a group object from type = [type0, type1, ...] and fields = [fields0, fields1, ...]
  // Resulting object:
  // {
  //    "_id": null,
  //    "__count__": {"$sum": 1},
  //    "field0$0": {"type0": "field0"},
  //    "field0$1": {"type1": "field1"},
  //    "field1$0": {"type0": "field0"},
  //    "field1$1": {"type1": "field11"},
  //    ...
  // }
  const getGroup = function(type, fields) {
    ...
    _.forEach(fields, (fieldName) => {
      _.forEach(type, (typeName, idx) => {
        const obj = {};
        obj[typeName] = `$${fieldName}`;
        group[`${fieldName}$${idx}`] = obj;
      });
    });
