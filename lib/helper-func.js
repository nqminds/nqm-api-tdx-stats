module.exports = (function() {
  ...
  // Returns a promise with the matched object
  // Creates the low and upp bin index values
  const getBinIndexPromise = function(result, params) {
    const left = result[params.field][0];
    const right = result[params.field][1];
    const matchObj = {count: result.count, matched: true};
    ...
    return Promise.resolve(matchObj);
  };
