module.exports = (function() {
  ...
  const getMinChunk = function(datasetId, params, cf) {
    ...
    const init = helper.getInitAccumulator(params.fields, params.type[0]);
    const processChunk = function(input, output, iterator) {
      output.count += input.count;

      _.forEach(params.fields, (field) => {
        output[field][params.type[0]] = Math.min(output[field][params.type[0]], input[field][params.type[0]]);
      });
  ...
  const getMaxChunk = function(datasetId, params, cf) {
    ...
    const processChunk = function(input, output, iterator) {
      ...
      _.forEach(params.fields, (field) => {
        output[field][params.type[0]] = Math.max(output[field][params.type[0]], input[field][params.type[0]]);
      });
  ...
  const getSumChunk = function(datasetId, params, cf) {
    ...
    const processChunk = function(input, output, iterator) {
      output.count += input.count;
      _.forEach(params.fields, (field) => {
        output[field][params.type[0]] += input[field][params.type[0]];
      });
  ...
  const getAvgChunk = function(datasetId, params, cf) {
    return getSumChunk(datasetId, params, cf)
            .then((output) => {
              ...
              let avgOutput = helper.getInitAccumulator(params.fields, params.type[0]);
              avgOutput[constants.countResultFieldName] = output[constants.countResultFieldName];
              if (output.count) {
                _.forEach(params.fields, (field) => {
                  avgOutput[field][params.type[0]] = output[field]["$sum"] / output.count;
                });
  ...
  const getStdChunk = function(datasetId, params, cf) {
    params.type = ["$stdDevPop"];
    const init = helper.getInitAccumulator(params.fields, params.type[0]);
  };

  const getMedChunk = function(datasetId, params, cf) {
