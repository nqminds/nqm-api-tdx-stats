module.exports = (function() {
  ...
  const indexChangeFunction = function() {
    ...
    const limit = this._internalParams.searchLimit;
    ...
    return aggregate(this._funcParams.datasetId, pipeline, null, this._funcParams.timeout)
            .then((val) => {
              this._internalParams.matchFilter =
                ...
                console.log(val.data[0].Contract+":"+val.data[0].NID+":"+val.data[0].First_Movement);
  ...
  function getChunk(type, datasetId, match, fields, index, timeout) {
    ...
    return api.getDatasetDataCountAsync(datasetId, match)
              .then((val) => {
                ...
                const iterations = parseInt(Math.floor(val.count / limit) + ((val.count % limit) ? 1 : 0));
                ...
                iterator.setInternalParam("searchLimit", limit);
  ...
  function FirstOrderChunkAPI(tdxApi, authFunc, slimit) {
    api = tdxApi;
    searchLimit = slimit;
